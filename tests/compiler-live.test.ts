...
test("live compile writes a guard only after eval proves it fires", () => {
  ...
  expect(compileLive(directory, gitStashUntrackedGuard, { chokepoint: "shell", command: "git stash -u", hasUnstagedChanges: true })).toEqual({ status: "pass" });
...
test("live compile writes to the selected repo or machine guard home", () => {
  ...
  expect(compileLiveForScope("repo", gitStashUntrackedGuard, { chokepoint: "shell", command: "git stash -u", hasUnstagedChanges: true }, environment, project)).toEqual({ status: "pass" });
  expect(readFileSync(join(project, ".vibebloat", "guards", "git-stash-u.json"), "utf8")).toContain("git-stash-u");
  expect(compileLiveForScope("machine", gitStashUntrackedGuard, { chokepoint: "shell", command: "git stash -u", hasUnstagedChanges: true }, environment, project)).toEqual({ status: "pass" });
