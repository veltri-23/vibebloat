...
test("installer wires verified shim, append-only hooks, and filesystem fallback", () => {
  const directory = mkdtempSync(join(process.env.TEMP ?? ".", "vibebloat-install-"));
  tempDirectories.push(directory);
  const claude = join(directory, "claude.json"); const codex = join(directory, "config.toml");
  ...
  const watcher = installNativeHooks({
    ...
    fallback: {
      ...
      gitHookPaths: [hook],
      fsWatchDirectory: directory,
      guards: [mcpConfigWrongFileGuard],
      onFsBlocked: () => {},
    },
  });
