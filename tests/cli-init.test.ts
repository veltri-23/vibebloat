...
test("unknown mode keeps the three-line error", () => {
  ...
  expect(result.stderr.toString()).toBe("WHAT failed: expected allow, compile, eval, hook, git-hook, disable, doctor, init, install, uninstall, scan, stats, sync, watch, or email.\nWHY: no supported mode supplied.\nFIX: bun src/cli.ts doctor\n");
});
